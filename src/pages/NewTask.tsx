...
const NewTask = () => {
  ...
  const handleAddPayment = () => {
    ...
    const paymentStage: PaymentStage = {
      ...
      payment_date: newPayment.due_date, // Set payment_date from the form field
  ...
  const handleCreatePartner = async () => {
    try {
      // Validate required fields
      if (
        !newPartner.first_name.trim() ||
        !newPartner.last_name.trim() ||
        !newPartner.email.trim() ||
        !newPartner.password.trim() ||
        !newPartner.mobile.trim() ||
        newPartner.mobile.length !== 10 ||
        !newPartner.partner_type ||
        !newPartner.license_id ||
        !newPartner.license_expiring_date
      ) {
        toast({
          title: "Error",
          description:
            "Please fill in all required fields (Name, Email, Password, Mobile, Partner Type, License ID, License Expiring Date)",
          variant: "destructive",
        });
        return;
      }
      ...
      // Hash password
      const hashedPassword = hashPassword(newPartner.password);

      // Auto-generate username from email
      const autoGeneratedUsername = newPartner.email;

      // Prepare API payload
      const requestBody = {
        first_name: newPartner.first_name,
        last_name: newPartner.last_name,
        email_address: newPartner.email,
        password: hashedPassword,
        username: autoGeneratedUsername,
        role: 'partner',
        mobile_number: newPartner.mobile,
        emergency_contact: newPartner.emergency_contact || newPartner.mobile,
        gender: newPartner.gender || "",
        age: newPartner.age ? parseInt(newPartner.age) : undefined,
        address: newPartner.address || "",
        partner_type: newPartner.partner_type,
        license_id: newPartner.license_id,
        license_expiring_date: newPartner.license_expiring_date,
        gstin: newPartner.gstin || "",
        pan: newPartner.pan_number || "",
      ...
      if (response.ok) {
        ...
        toast({
          ...
          description: `${newPartner.first_name} ${newPartner.last_name} has been created successfully`,
        });
      } else {
        const message = (responseBody && (responseBody.message || responseBody.error || JSON.stringify(responseBody))) || 'Failed to create partner';
        toast({
          title: "Error",
          description: message,
          variant: "destructive",
      ...
      toast({
        title: "Error",
        description: "Unexpected error creating partner",
        variant: "destructive",
      });
    }
  };
  ...
  const createTaskData = (): TaskCreateRequest => {
    ...
    const taskPayments = paymentStages.map((payment) => {
      ...
      payment_date: payment.payment_date || payment.due_date, // Send payment_date instead of due_date
    ...
    const taskData = {
      ...
      assignedTo: formData.assigned_to && formData.assigned_to.trim() !== '' ? formData.assigned_to : null,
      priority: formData.priority,
      ticket_Stage: formData.ticket_stage,
      partner_id: formData.partner_id && formData.partner_id.trim() !== '' ? formData.partner_id : null,
      dueDate: formData.due_date && formData.due_date.trim() !== '' ? formData.due_date : null,
      stakeholders: taskStakeholders,
      customer: customerData,
      comments: comment.text,
      internal: comment.isInternal ? "true" : "false",
      payments: taskPayments,
      referring_partner_id: formData.partner_id && formData.partner_id.trim() !== '' ? formData.partner_id : null, // Use the selected partner ID or null
                ...
                <div>
                  ...
                  <Popover open={partnerComboboxOpen} onOpenChange={setPartnerComboboxOpen}>
                    <PopoverTrigger asChild>
                      ...
                      >
                        {formData.partner_id
                          ? partners.find((p) => p.partner_id.toString() === formData.partner_id)
                            ? `${partners.find((p) => p.partner_id.toString() === formData.partner_id)?.first_name} ${partners.find((p) => p.partner_id.toString() === formData.partner_id)?.last_name}`
                            : "Select partner"
                          : "Select partner"}
                            ...
                            {[...partners]
                              ...
                              .map((partner) => (
                                <CommandItem
                                  ...
                                  onSelect={() => {
                                    handleSelectChange("partner_id", partner.partner_id.toString());
                                ...
                                >
                                  <Check
                                    className={cn(
                                      "mr-2 h-4 w-4",
                                      formData.partner_id === partner.partner_id.toString()
                                        ? "opacity-100"
                                        : "opacity-0"
                                    )}
                                  />
                        ...
                        <div className="rounded-lg border bg-white p-4 sm:p-5 shadow-sm">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                              ...
                              <Input
                                value={newPartner.emergency_contact}
                                onChange={(e) =>
                                  setNewPartner({
                                    ...newPartner,
                        ...
                        <div className="rounded-lg border bg-white p-4 sm:p-5 shadow-sm">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                              ...
                              <Input
                                type="date"
                                value={newPartner.license_expiring_date}
                                onChange={(e) =>
                                  setNewPartner({
                                    ...newPartner,
                                    license_expiring_date: e.target.value,
                                  })
                                }
                              />
                            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Popover open={assignedToComboboxOpen} onOpenChange={setAssignedToComboboxOpen}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        role="combobox"
                        aria-expanded={assignedToComboboxOpen}
                        className="w-full justify-between"
                        disabled={isLoadingUsers}
                      >
                        {formData.assigned_to
                          ? users.find((u) => u.id === formData.assigned_to)
                            ? users.find((u) => u.id === formData.assigned_to)?.full_name
                            : "Select assignee"
                          : "Select assignee"}
                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                      </Button>
                                ...
                                {[...users]
                                  ...
                                  .map((user) => (
                                    <CommandItem
                                      ...
                                      onSelect={() => {
                                        handleSelectChange("assigned_to", user.id);
                                        setAssignedToComboboxOpen(false);
                                      }}
                                    >
                                      <Check
                                        className={cn(
