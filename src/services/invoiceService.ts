...
export const generateInvoiceNumber = (): string => {
  // Default format: ECSI-YY-0080 (starting from 0080)
  const year = new Date().getFullYear().toString().slice(-2);
  return `ECSI-${year}-0080`;
};
...
export const prepareInvoiceData = (paymentStage: any, caseData: any, customerData: any): InvoiceData => {
  ...
  // Resolve customer name using multiple possible field names found across the app
  const customerName = (
    customerData?.customer_name ||
    customerData?.full_name ||
    customerData?.name ||
    ((customerData?.first_name || customerData?.last_name) ? `${customerData?.first_name || ''} ${customerData?.last_name || ''}`.trim() : '') ||
  ...
  const customerCity = customerData?.city || customerData?.town || '';
  const customerState = customerData?.state || customerData?.region || '';
  const customerPincode = customerData?.pincode || customerData?.zip || customerData?.postal_code || '';
  ...
  return {
    ...
    customerDetails: {
      ...
      address: customerAddress || 'Not added address',
      city: customerCity || 'Not added city',
      state: customerState || 'Not added state',
      pincode: customerPincode || 'Not added pincode',
      gstNumber: customerData?.gstin || customerData?.gst_number,
      panNumber: customerData?.pan || customerData?.pan_number,
      phone: customerData?.mobile_number || customerData?.phone || customerData?.contact_number,
      email: customerData?.email_address || customerData?.email
    },
    shipToDetails: {
      name: customerName || 'Not added name',
      address: customerAddress || 'Not added address',
    ...
    caseDetails: {
      description: 'Claim Assistance Services- Fee',
      caseNumber: caseData?.case_number || 'N/A'
    },
    totals: { subtotal, cgstTotal, sgstTotal, igstTotal, grandTotal },
    paymentTerms: 'Payment due within 30 days of invoice date.',
  ...
  <div class="invoice-container">
   ...
   <table style="margin-bottom:0; border-bottom:none;">
      ...
      <tbody>
        ...
        <tr>
          <td style="font-weight:bold">
            ${data.customerDetails.name || 'Not added name'},<br/>
            ${data.customerDetails.address || 'Not added address'},<br/>
            ${data.customerDetails.pincode || 'Not added pincode'}<br/>
            ${data.customerDetails.state || 'Not added state'}
          </td>
          <td style="font-weight:bold">
            ${data.companyDetails.name},<br/>
            Plot No. 12, Road No. 1,<br/>
    ...
    <table style="margin-top:0;">
      ...
      <tbody>
        ...
        <tr>
          <td></td><td></td>
          <td class="text-center">CGST @9%</td>
          <td class="text-right">${data.totals.cgstTotal > 0 ? data.totals.cgstTotal.toLocaleString('en-IN') : ''}</td>
        </tr>
        <tr>
          <td></td><td></td>
          <td class="text-center">SGST @9%</td>
          <td class="text-right">${data.totals.sgstTotal > 0 ? data.totals.sgstTotal.toLocaleString('en-IN') : ''}</td>
        </tr>
        <tr>
          <td></td><td></td>
          <td class="text-center">IGST @18%</td>
          <td class="text-right">${data.totals.igstTotal > 0 ? data.totals.igstTotal.toLocaleString('en-IN') : ''}</td>
        </tr>
        <tr>
          <td></td><td></td>
          <td class="text-center">Gross Amount</td>
    ...
    <div style="">
      ...
      <div class="font-bold">Account No. 42310776320</div>
      <div class="font-bold">Name of Bank: State Bank of India</div>
      <div class="font-bold">Name of Branch: Kondapur, Hyderabad</div>
      <div class="font-bold">IFSC Code: SBIN0018655</div>
    </div>
    ...
    <div class="footer-info">
      <div>Head Off: ${data.companyDetails.address}, ${data.companyDetails.city} - ${data.companyDetails.pincode}</div>
      <div>Regd Office: Flat No 403, Apex Sai Srinivasam, Rama Krishna Nagar, Kondapur, Hyderabad-500 050</div>
      <div>E-Mail Id: support@expertclaims.co.in | Mob No. +919985060600</div>
    </div>
  </div>
</body>
...
export const generateAndDownloadInvoice = async (
  ...
  try {
    const baseData = prepareInvoiceData(paymentStage, caseData, customerData);

    const invoiceData: InvoiceData = {
      ...baseData,
      invoiceNumber: overrides?.invoiceNumber ?? baseData.invoiceNumber,
      invoiceDate: overrides?.invoiceDate ?? baseData.invoiceDate,
      caseDetails: {
        ...baseData.caseDetails,
        description: overrides?.description ?? baseData.caseDetails.description
      }
    };
